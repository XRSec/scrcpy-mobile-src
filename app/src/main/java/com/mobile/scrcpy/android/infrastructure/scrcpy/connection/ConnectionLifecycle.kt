...
class ConnectionLifecycle(
    ...
    ): Result<Pair<VideoStream?, AudioStream?>> = withContext(Dispatchers.IO) {
        try {
            // 步骤 1: 验证 ADB 连接
            if (!skipAdbConnect) {
            ...
            startScrcpyServer(
                connection, scid, maxSize, bitRate, maxFps,
                videoCodec, videoEncoder, enableAudio, audioCodec, audioEncoder,
                stayAwake, powerOffOnClose
            )
            ...
            if (deviceId != null) {
                ...
                if (connection != null) {
                    try {
                        ...
                        LogManager.w(LogTags.SCRCPY_CLIENT, "${RemoteTexts.SCRCPY_REMOVE_FORWARD_FAILED.get()}: ${e.message}")
                    }
                }
            }
            ...
            if (deviceId != null && currentScid != null) {
                ...
                if (connection != null) {
                    try {
                        ...
                        LogManager.d(LogTags.SCRCPY_CLIENT, "✓ ${RemoteTexts.SCRCPY_TERMINATED_SERVER_PROCESS.get()} (scid=$scidHex)")
                    } catch (e: Exception) {
                        LogManager.w(LogTags.SCRCPY_CLIENT, "${RemoteTexts.SCRCPY_TERMINATE_SERVER_FAILED.get()}: ${e.message}")
                    }
                }
            }
            ...
            if (deviceId.startsWith("usb:")) {
                ...
                if (parts.size == 2) {
                    ...
                    if (reconnectResult.isFailure) {
                        throw Exception("${AdbTexts.ERROR_ADB_RECONNECT_FAILED.get()}: ${reconnectResult.exceptionOrNull()?.message}")
                    }
    ...
    private suspend fun cleanupOldResources(
        ...
        try {
            ...
            if (currentScid != null) {
                ...
                connection.executeShell(killCmd, retryOnFailure = false)
                LogManager.d(LogTags.SCRCPY_CLIENT, "✓ ${RemoteTexts.SCRCPY_CLEANED_OLD_SERVER_PROCESS.get()} (scid=$oldScidHex)")
            }
            delay(200)
        } catch (e: Exception) {
            LogManager.w(LogTags.SCRCPY_CLIENT, "${RemoteTexts.SCRCPY_CLEANUP_OLD_RESOURCES_FAILED.get()}: ${e.message}")
        }
    }
    ...
    private suspend fun startScrcpyServer(
        ...
        val command = buildScrcpyCommand(
            maxSize, bitRate, maxFps, scid, videoCodec, videoEncoder,
            enableAudio, audioCodec, audioEncoder, stayAwake, powerOffOnClose
        )
