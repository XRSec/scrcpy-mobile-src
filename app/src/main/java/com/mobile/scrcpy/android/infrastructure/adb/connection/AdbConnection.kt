...
class AdbConnection(
    ...
    suspend fun executeShell(command: String, retryOnFailure: Boolean = true): Result<String> = withContext(Dispatchers.IO) {
        try {
            ...
            LogManager.d(LogTags.ADB_CONNECTION, "${AdbTexts.ADB_DISCONNECTED_ECONNREFUSED.get()} (ECONNREFUSED)，${AdbTexts.ADB_CANNOT_EXECUTE_COMMAND.get()}: $command - ${e.message}")
            ...
            if (retryOnFailure) {
                ...
                try {
                    ...
                    LogManager.d(LogTags.ADB_CONNECTION, "${AdbTexts.ADB_AUTO_RECONNECT_STILL_FAILED.get()}: ${retryException.message}")
                    Result.failure(retryException)
                }
            } else {
                LogManager.d(LogTags.ADB_CONNECTION, "${AdbTexts.ADB_CONNECTION_CLOSED.get()}，${AdbTexts.ADB_CANNOT_EXECUTE_COMMAND.get()}: $command")
                Result.failure(e)
            }
        } catch (e: SocketException) {
            // Socket 异常，检查是否是 ECONNREFUSED
            if (e.message?.contains("ECONNREFUSED", ignoreCase = true) == true) {
                LogManager.d(LogTags.ADB_CONNECTION, "${AdbTexts.ADB_SOCKET_EXCEPTION.get()} (ECONNREFUSED): $command - ${e.message}")
                Result.failure(Exception(AdbTexts.ERROR_ADB_CONNECTION_DISCONNECTED.get(), e))
            } else if (retryOnFailure) {
                // 其他 Socket 异常，尝试重连
                LogManager.d(LogTags.ADB_CONNECTION, "${AdbTexts.ADB_SOCKET_EXCEPTION_RETRY.get()}: $command - ${e.message}")
                try {
                    delay(100) // 短暂延迟，让 dadb 完成重连
                    val retryResponse = dadb.shell(command)
                    LogManager.d(LogTags.ADB_CONNECTION, "✓ ${AdbTexts.ADB_AUTO_RECONNECT_SUCCESS.get()}")
                ...
                LogManager.d(LogTags.ADB_CONNECTION, "${AdbTexts.ADB_SOCKET_EXCEPTION.get()}: $command - ${e.message}")
                Result.failure(e)
            }
        } catch (e: Exception) {
    ...
    suspend fun setupPortForward(localPort: Int, remotePort: Int): Result<Boolean> = withContext(Dispatchers.IO) {
        try {
            ...
            LogManager.d(LogTags.ADB_CONNECTION, "${AdbTexts.ADB_PORT_FORWARD_SUCCESS.get()}: $localPort -> $remotePort")
            Result.success(true)
        } catch (e: Exception) {
    ...
    suspend fun setupAdbForward(localPort: Int, socketName: String): Result<Boolean> = withContext(Dispatchers.IO) {
        try {
            ...
            try {
                ...
                LogManager.d(LogTags.ADB_CONNECTION, "${AdbTexts.ADB_FORWARD_SETUP_SUCCESS.get()}（${CommonTexts.LABEL_USING.get()} SocketForwarder: $localPort -> $targetSocket）")
                Result.success(true)
            } catch (e: Exception) {
                LogManager.e(LogTags.ADB_CONNECTION, "${AdbTexts.ADB_SOCKET_FORWARDER_FAILED.get()}: ${e.message}", e)
                Result.failure(e)
            }
        } catch (e: Exception) {
     ...
     * 推送文件
     */
    suspend fun pushFile(localPath: String, remotePath: String): Result<Boolean> {
        return AdbFileOperations.pushFile(dadb, localPath, remotePath)
    }
    
    /**
     * 拉取文件
     */
    suspend fun pullFile(remotePath: String, localPath: String): Result<Boolean> {
        return AdbFileOperations.pullFile(dadb, remotePath, localPath)
    }
    
    /**
     * 安装 APK
     */
    suspend fun installApk(apkPath: String): Result<Boolean> {
        return AdbFileOperations.installApk(dadb, apkPath)
    }
    ...
    suspend fun uninstallPackage(packageName: String): Result<Boolean> {
        return AdbFileOperations.uninstallPackage(dadb, packageName)
    }
    
    /**
     * 推送 scrcpy-server.jar 到设备
     */
    suspend fun pushScrcpyServer(context: Context, scrcpyServerPath: String = "/data/local/tmp/scrcpy-server.jar"): Result<Boolean> {
        return AdbFileOperations.pushScrcpyServer(dadb, context, scrcpyServerPath)
    }
    ...
    suspend fun detectVideoEncoders(context: Context): Result<List<VideoEncoderInfo>> {
        return AdbEncoderDetector.detectVideoEncoders(dadb, context, ::openShellStream)
    }
    ...
    suspend fun detectAudioEncoders(context: Context): Result<List<AudioEncoderInfo>> {
        return AdbEncoderDetector.detectAudioEncoders(dadb, context, ::openShellStream)
    }
    ...
    fun close() {
        try {
            ...
            LogManager.e(LogTags.ADB_CONNECTION, "${AdbTexts.ADB_CLOSE_CONNECTION_FAILED_DETAIL.get()}: ${e.message}", e)
        }
    }
}
