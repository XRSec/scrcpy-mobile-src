...
object AdbFileOperations {
     ...
     * 推送文件
     */
    suspend fun pushFile(dadb: Dadb, localPath: String, remotePath: String): Result<Boolean> = withContext(Dispatchers.IO) {
        try {
            val file = File(localPath)
            dadb.push(file, remotePath)
            LogManager.d(LogTags.ADB_CONNECTION, "${AdbTexts.ADB_FILE_PUSH_SUCCESS.get()}: $localPath -> $remotePath")
            Result.success(true)
        } catch (e: Exception) {
     ...
     * 拉取文件
     */
    suspend fun pullFile(dadb: Dadb, remotePath: String, localPath: String): Result<Boolean> = withContext(Dispatchers.IO) {
        try {
            val file = File(localPath)
            dadb.pull(file, remotePath)
            LogManager.d(LogTags.ADB_CONNECTION, "${AdbTexts.ADB_FILE_PULL_SUCCESS.get()}: $remotePath -> $localPath")
            Result.success(true)
        } catch (e: Exception) {
     ...
     * 安装 APK
     */
    suspend fun installApk(dadb: Dadb, apkPath: String): Result<Boolean> = withContext(Dispatchers.IO) {
        try {
     ...
     * 卸载应用
     */
    suspend fun uninstallPackage(dadb: Dadb, packageName: String): Result<Boolean> = withContext(Dispatchers.IO) {
        try {
    ...
    suspend fun pushScrcpyServer(
        ...
        try {
            try {
                ...
                return@withContext Result.failure(Exception(AdbTexts.ADB_SCRCPY_SERVER_NOT_IN_ASSETS.get() + ": ${e.message}"))
            }
