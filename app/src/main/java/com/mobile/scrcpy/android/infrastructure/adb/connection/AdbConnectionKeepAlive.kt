...
internal class AdbConnectionKeepAlive(
    ...
    fun start(getConnections: () -> List<AdbConnection>) {
            ...
            keepAliveScope.launch {
                while (isActive) {
                    ...
                    getConnections().forEach { connection ->
                        ...
                        if (result.isSuccess) {
                            ...
                            LogManager.w(
                                ...
                                "${AdbTexts.ADB_HEARTBEAT_FAILED.get()}: ${connection.deviceId} - ${error?.message}",
                            )
                            ...
                            if (error?.message?.contains(AdbTexts.ERROR_ADB_CONNECTION_DISCONNECTED.get()) == true) {
                                LogManager.e(
                                    LogTags.ADB_CONNECTION,
                                    "${AdbTexts.ADB_CONNECTION_DETECTED_DISCONNECTED.get()}: ${connection.deviceId}",
                                )
                    ...
                    failedConnections.forEach { deviceId ->
                        LogManager.d(
                            LogTags.ADB_CONNECTION,
                            "${AdbTexts.ADB_CLEANUP_INVALID_CONNECTION.get()}: $deviceId",
                        )
        ...
        LogManager.d(
            LogTags.ADB_CONNECTION,
            "${AdbTexts.ADB_KEEPALIVE_STARTED.get()}（${CommonTexts.LABEL_INTERVAL.get()}: ${keepAliveInterval}ms）",
        )
    }
